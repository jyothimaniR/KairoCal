...
class AdvancedEventPriorityClassifier:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        # Prefer local tokenizer if model_path provided (offline-friendly)
        try:
            ...
            # Last resort: initialize from base name (may require network)
            self.tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
    ...
    def _initialize_new_model(self):
        ...
        # Initialize from config to avoid remote fetches
        base_config = DistilBertConfig()
        self.bert_model = DistilBertModel(base_config)
    ...
    def _load_trained_model(self, model_path: str):
        ...
        if os.path.exists(checkpoint_path):
            ...
            # Initialize from config locally to avoid network
            base_config = DistilBertConfig()
            self.bert_model = DistilBertModel(base_config)
            self.feature_combiner = nn.Sequential(
